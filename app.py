...
def wykonaj_analize_krs(krs, log_callback):
    ...
    if os.path.exists(katalog_pobrane): shutil.rmtree(katalog_pobrane)
    ...
    with SB(uc=False, browser="chrome", binary_location=executable_path, headless=True, xvfb=True,
        ...
        try:
            log_callback("🚀 Łączenie z bazą RDF Ministerstwa...")
            ...
            time.sleep(2)
            try: sb.click('button:contains("Akceptuj")', timeout=3)
            except: pass 
            ...
            log_callback(f"📊 Zidentyfikowano {len(znalezione_pozycje)} sprawozdań.")

            pobrane_archiwa = []
            for idx, pos in enumerate(znalezione_pozycje):
                log_callback(f"📥 Pobieranie dokumentu {idx+1}...")
                btn = f"tbody tr:nth-child({pos}) button"
                sb.click(btn)
                time.sleep(2)
                sb.click("span.p-button-label:contains('Pobierz dokumenty')")

                # Czekamy na zakończenie pobierania - sprawdzamy rozmiar pliku
                sciezka_docelowa = ""
                for _ in range(30):
                    time.sleep(1)
                    ...
                    if zips:
                        ...
                        # Jeśli plik przestał rosnąć, uznajemy go za pobrany
                        sciezka_docelowa = os.path.join(katalog_pobrane, f"analiza_{idx}.zip")
                        os.rename(najnowszy, sciezka_docelowa)
                        pobrane_archiwa.append(sciezka_docelowa)
                        break
                sb.click(btn) # Zwiń
...
krs = st.sidebar.text_input("Numer KRS", max_chars=10)
if st.sidebar.button("Analizuj"):
    with st.status("Trwa praca bota...", expanded=True) as status:
        l_area = st.empty()
        l_list = []
        def my_log(m):
    ...
    if nazwa:
        ...
        if dane:
            ...
            st.table(df_v)
            st.metric("Suma", f"{df['Podatek (RB)'].sum():,.2f} PLN")
        else:
            st.warning("Nie znaleziono tagów finansowych w pobranych plikach.")
